...
export class GenreSequelizeRepository implements IGenreRepository {
  ...
  async existsByIds(
    ...
    const notExistsGenreIds = ids.filter(
      (id) => !existsGenreIds.some((existsId) => existsId.equals(id)),
    );
  ...
  async update(entity: Genre): Promise<void> {
    const model = await this._get(entity.genre_id.id);
    if (!model) {
    ...
    await this.genreModel.update(GenreModelMapper.toModelProps(entity), {
  ...
  async search(props: GenreSearchParams): Promise<GenreSearchResult> {
    ...
    // @ts-ignore - count is a number
    const count: number = await this.genreModel.count({
      distinct: true,
      // @ts-ignore - add include only if categories_id is in filter
    ...
    const models = await this.genreModel.findAll({
      where: {
        genre_id: {
          [Op.in]: idsResult.map((r: any) => r.genre_id),
        },
      },
      include: ['categories_id'],
