  ...
  describe('transaction mode', () => {
    ...
    describe('bulkInsert method', () => {
      ...
      it('rollback the bulk insertion', async () => {
        ...
        await expect(genreRepo.findAll()).resolves.toHaveLength(0);
      });
    });
    ...
    describe('existsByIds method', () => {
      it('should return true when genres exists', async () => {
        const category = Category.fake().aCategory().build();
        await categoryRepo.insert(category);
        const genres = Genre.fake()
          .aGenre()
        ...
        const result = await genreRepo.existsByIds([genres.genre_id]);

        expect(result.exists).toHaveLength(1);
        expect(result.exists[0]).toBeValueObject(genres.genre_id);
    ...
    describe('delete method', () => {
      ...
      it('should rollback the delete', async () => {
        const category = Category.fake().aCategory().build();
        await categoryRepo.insert(category);
        const genre = Genre.fake()
          .aGenre()
        ...
        await expect(
          genreRepo.findById(genre.genre_id),
